...
extern uint32_t TXBuff[10],TYBuff[10];     //定义两个数组存放前面的点做缓存滤波
...
void LCD_Touch_OnDown(void);
void LCD_Touch_OnUp(void);
LCD_TouchState LCD_Touch_GetState(void);

/* LCD Touch Draw */
void LCD_Touch_Draw_ConnectLastPoint(const LCD_TouchPoint* p);
void LCD_Touch_Draw_PrintInfo(void);
void LCD_Touch_Draw_OnUp(void);
void LCD_Touch_Draw_Update(void);
